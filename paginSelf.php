...
<?php while ($row=$stmt->fetch(PDO::FETCH_ASSOC))
    ...
    <form name="prod" action="gestion.php" method="post"><tr>
        ...
        <td><input type="image" name="zoom" src="TableauProd/boutons/search.png" alt="search"></td>
    </tr></form>
<?php } ?>
</table>
...
<form name="nav" action="paginSelf.php" method="post">
	...
	<input type="image" name="first" src="TableauProd/boutons/first.png" alt="first" />
    <?php } if($_SESSION['nbpage'] !== 0) {?>
	<input type="image" name="previous" src="TableauProd/boutons/previous.png" alt="previous" />
    <?php } if($_SESSION['nbpage'] < ($nbpages)) {?>
	<input type="image" name="next" src="TableauProd/boutons/next.png" alt="next" />
    <?php } if($_SESSION['nbpage'] < ($nbpages-1)) {?>
	<input type="image" name="last" src="TableauProd/boutons/last.png" alt="last" />
    <?php } ?>
</form>